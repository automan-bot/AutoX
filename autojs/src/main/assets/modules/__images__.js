module.exports = function (runtime, scope) {
    ...
    MatchingResult.prototype.sortBy = function (cmp) {
      ...
      if (typeof cmp == "string") {
        cmp.split("-").forEach((direction) => {
          ...
          if (!buildInFn) {
            throw new Error(
              "unknown direction '" + direction + "' in '" + cmp + "'"
            );
          }
          (function (fn) {
  ...
  if (android.os.Build.VERSION.SDK_INT >= 21) {
    util.__assignFunctions__(runtime.images, images, [
      "captureScreen",
      "read",
      "copy",
      "load",
  ...
  with (images.opencvImporter) {
    ...
    colors.isSimilar = function (c1, c2, threshold, algorithm) {
      ...
      return colorDetector.detectsColor(
        colors.red(c2),
        colors.green(c2),
        colors.blue(c2)
      );
    ...
    images.requestScreenCapture = function (landscape) {
      let ScreenCapturer =
        com.stardust.autojs.core.image.capture.ScreenCapturer;
    ...
    images.inRange = function (img, lowerBound, upperBound) {
      initIfNeeded();
      var lb = new Scalar(
        colors.red(lowerBound),
        colors.green(lowerBound),
        colors.blue(lowerBound),
        colors.alpha(lowerBound)
      );
      var ub = new Scalar(
        colors.red(upperBound),
        colors.green(upperBound),
        colors.blue(upperBound),
        colors.alpha(lowerBound)
      );
      var bi = new Mat();
    ...
    images.adaptiveThreshold = function (
      img,
      maxValue,
      adaptiveMethod,
      thresholdType,
    ...
    images.findCircles = function (grayImg, options) {
      ...
      var mat =
        options.region == undefined
          ? grayImg.mat
          : new Mat(grayImg.mat, buildRegion(options.region, grayImg));
      var resultMat = new Mat();
      var dp = options.dp == undefined ? 1 : options.dp;
      var minDst =
        options.minDst == undefined ? grayImg.height / 8 : options.minDst;
    ...
    images.concat = function (img1, img2, direction) {
      ...
      return javaImages.concat(
        img1,
        img2,
        android.view.Gravity[direction.toUpperCase()]
      );
    };
    ...
    images.findColor = function (img, color, options) {
      ...
      if (options.region) {
        return colorFinder.findColor(
          ...
          color,
          threshold,
          buildRegion(options.region, img)
        );
      } else {
    ...
    images.findColorInRegion = function (
      img,
      color,
      x,
      y,
    ...
    images.findAllPointsForColor = function (img, color, options) {
      ...
      if (options.region) {
        return toPointArray(
          colorFinder.findAllPointsForColor(
            img,
            color,
            threshold,
            buildRegion(options.region, img)
        ...
        return toPointArray(
          colorFinder.findAllPointsForColor(img, color, threshold, null)
        );
      }
    };
    ...
    images.findMultiColors = function (img, firstColor, paths, options) {
      ...
      var list = java.lang.reflect.Array.newInstance(
      ...
      var threshold =
        options.threshold === undefined
          ...
          : options.threshold;
      return colorFinder.findMultiColors(
        img,
        firstColor,
        threshold,
        region,
    ...
    images.matchTemplate = function (img, template, options) {
      ...
      var result;
      if (options.region) {
        result = javaImages.matchTemplate(
          img,
          template,
          weakThreshold,
          threshold,
    ...
    images.findImageInRegion = function (
      img,
      template,
      x,
      y,
    ...
    function getColorDetector(color, algorithm, threshold) {
      switch (algorithm) {
        case "rgb":
          return new com.stardust.autojs.core.image.ColorDetector.RGBDistanceDetector(
            color,
            threshold
          );
        case "equal":
          return new com.stardust.autojs.core.image.ColorDetector.EqualityDetector(
            color
          );
        case "diff":
          return new com.stardust.autojs.core.image.ColorDetector.DifferenceDetector(
            color,
            threshold
          );
        case "rgb+":
          return new com.stardust.autojs.core.image.ColorDetector.WeightedRGBDistanceDetector(
            color,
            threshold
          );
        case "hs":
          return new com.stardust.autojs.core.image.ColorDetector.HSDistanceDetector(
            color,
            threshold
          );
      }
